...
(2, 999999,'excel_process', 'excel_process', 'default_icon/default_workflow_icon.png', 1750254785913,3, 0, 0,  0, 0, 0)
ON DUPLICATE KEY UPDATE
id = VALUES(id);

INSERT INTO workflow_draft (id, canvas, input_params, output_params, test_run_success, modified, updated_at, deleted_at, commit_id) VALUES (2, '{"nodes":[{"id":"100001","type":"1","meta":{"position":{"x":-106.5,"y":-1.4210854715202004e-14}},"data":{"nodeMeta":{"description":"工作流的起始节点，用于设定启动工作流需要的信息","icon":"https://lf3-static.bytednsdoc.com/obj/eden-cn/dvsmryvd_avi_dvsm/ljhwZthlaukjlkulzlp/icon/icon-Start-v2.jpg","subTitle":"","title":"开始"},"outputs":[{"type":"string","assistType":7,"name":"input","required":true},{"type":"string","name":"sandbox_id","required":true},{"type":"string","name":"url","required":true}],"trigger_parameters":[{"type":"string","assistType":7,"name":"input","required":true},{"type":"string","name":"sandbox_id","required":true},{"type":"string","name":"url","required":true}]}},{"id":"900001","type":"2","meta":{"position":{"x":1255.6000000000001,"y":-13.000000000000014}},"data":{"nodeMeta":{"description":"工作流的最终节点，用于返回工作流运行后的结果信息","icon":"https://lf3-static.bytednsdoc.com/obj/eden-cn/dvsmryvd_avi_dvsm/ljhwZthlaukjlkulzlp/icon/icon-End-v2.jpg","subTitle":"","title":"结束"},"inputs":{"terminatePlan":"returnVariables","inputParameters":[{"name":"output","input":{"type":"string","value":{"type":"literal","content":"1","rawMeta":{"type":1}}}}]}}},{"id":"109520","type":"5","meta":{"position":{"x":367.5999999999999,"y":-13.700000000000017}},"data":{"nodeMeta":{"title":"代码","icon":"https://lf3-static.bytednsdoc.com/obj/eden-cn/dvsmryvd_avi_dvsm/ljhwZthlaukjlkulzlp/icon/icon-Code-v2.jpg","description":"编写代码，处理输入变量来生成返回值","mainColor":"#00B2B2","subTitle":"代码"},"inputs":{"inputParameters":[{"name":"input","input":{"type":"string","assistType":7,"value":{"type":"ref","content":{"source":"block-output","blockID":"100001","name":"input"},"rawMeta":{"type":13}}}}],"code":"import urllib.request\\nimport zipfile\\nimport xml.etree.ElementTree as ET\\nfrom io import BytesIO\\nfrom typing import Dict, List, Any\\n\\n\\ndef extract_excel_data(excel_url: str, target_columns: List[str]) -> Dict[str, Any]:\\n    # 1. 清理URL\\n    cleaned_url = excel_url.strip().replace(\\"\\\\n\\", \\"\\").replace(\\"\\\\r\\", \\"\\").replace(\\" \\", \\"\\")\\n    if not cleaned_url.startswith((\\"http://\\", \\"https://\\")):\\n        raise ValueError(f\\"无效的URL格式: {cleaned_url}\\")\\n\\n    # 2. 下载Excel文件\\n    try:\\n        with urllib.request.urlopen(cleaned_url, timeout=30) as response:\\n            if response.status != 200:\\n                raise Exception(f\\"下载失败，状态码: {response.status}\\")\\n            xlsx_data = BytesIO(response.read())\\n    except Exception as e:\\n        raise Exception(f\\"下载文件出错: {str(e)}\\")\\n\\n    # 3. 解压并解析文件\\n    try:\\n        with zipfile.ZipFile(xlsx_data, \\"r\\") as zf:\\n            sheet_files = [f for f in zf.namelist() if f.startswith(\\"xl/worksheets/sheet\\") and f.endswith(\\".xml\\")]\\n            if not sheet_files:\\n                raise Exception(\\"未找到工作表文件\\")\\n\\n            # 读取共享字符串表\\n            shared_strings = []\\n            if \\"xl/sharedStrings.xml\\" in zf.namelist():\\n                with zf.open(\\"xl/sharedStrings.xml\\") as f:\\n                    ss_xml = f.read().decode(\\"utf-8\\", errors=\\"replace\\")\\n                ss_root = ET.fromstring(ss_xml)\\n                for si in ss_root.iter(\\"{http://schemas.openxmlformats.org/spreadsheetml/2006/main}si\\"):\\n                    text_parts = []\\n                    for t in si.iter(\\"{http://schemas.openxmlformats.org/spreadsheetml/2006/main}t\\"):\\n                        if t.text:\\n                            text_parts.append(t.text)\\n                    shared_strings.append(\\"\\".join(text_parts))\\n\\n            # 读取Sheet1内容\\n            with zf.open(sheet_files[0]) as f:\\n                xml_content = f.read().decode(\\"utf-8\\", errors=\\"replace\\")\\n            root = ET.fromstring(xml_content)\\n            row_tag = \\"{http://schemas.openxmlformats.org/spreadsheetml/2006/main}row\\"\\n            cell_tag = \\"{http://schemas.openxmlformats.org/spreadsheetml/2006/main}c\\"\\n            value_tag = \\"{http://schemas.openxmlformats.org/spreadsheetml/2006/main}v\\"\\n\\n    except Exception as e:\\n        raise Exception(f\\"解压/解析文件出错: {str(e)}\\")\\n\\n    # 4. 建立列名与单元格地址的映射（关键修复：用Excel地址映射而非索引）\\n    column_address_map: Dict[str, str] = {}  # 存储列名 -> 单元格地址（如\\"A1\\", \\"B1\\"）\\n    rows = list(root.iter(row_tag))\\n    if not rows:\\n        raise Exception(\\"工作表中没有数据\\")\\n\\n    # 解析表头行（第一行），记录每个目标列的实际Excel地址\\n    header_row = rows[0]\\n    for cell in header_row.iter(cell_tag):\\n        cell_address = cell.get(\\"r\\", \\"\\")  # 如\\"B1\\"\\n        cell_type = cell.get(\\"t\\", \\"\\")\\n        value_elem = cell.find(value_tag)\\n        raw_value = value_elem.text if (value_elem is not None and value_elem.text) else \\"\\"\\n\\n        # 解析表头值\\n        if cell_type == \\"s\\":\\n            try:\\n                s_idx = int(raw_value)\\n                cell_value = shared_strings[s_idx] if s_idx < len(shared_strings) else raw_value\\n            except:\\n                cell_value = raw_value\\n        else:\\n            cell_value = raw_value.strip()\\n\\n        # 记录列名对应的单元格地址（如\\"商品组\\" -> \\"J1\\"）\\n        if cell_value in target_columns:\\n            # 提取列地址前缀（如从\\"B1\\"提取\\"B\\"）\\n            col_address = \\"\\".join([c for c in cell_address if c.isalpha()])\\n            column_address_map[cell_value] = col_address\\n\\n    # 验证所有目标列都被找到\\n    missing_cols = [col for col in target_columns if col not in column_address_map]\\n    if missing_cols:\\n        raise ValueError(f\\"未找到列名: {missing_cols}，请检查Excel表头是否正确\\")\\n\\n    # 5. 提取目标列数据（按地址映射提取，避免错位）\\n    raw_data: Dict[str, Dict[str, Any]] = {}\\n    for row_idx, row in enumerate(rows, start=1):\\n        # 跳过表头行\\n        if row_idx == 1:\\n            continue\\n\\n        row_data: Dict[str, Any] = {}\\n        row_num = str(row_idx)\\n        # 按列地址映射提取数据\\n        for col_name, col_address in column_address_map.items():\\n            # 构建当前行的单元格地址（如\\"B\\"列 + 行号2 -> \\"B2\\"）\\n            target_cell_address = f\\"{col_address}{row_num}\\"\\n            found = False\\n\\n            # 遍历当前行的所有单元格，找到目标地址的单元格\\n            for cell in row.iter(cell_tag):\\n                if cell.get(\\"r\\", \\"\\") == target_cell_address:\\n                    # 找到目标单元格，解析值\\n                    cell_type = cell.get(\\"t\\", \\"\\")\\n                    value_elem = cell.find(value_tag)\\n                    raw_value = value_elem.text if (value_elem is not None and value_elem.text) else \\"\\"\\n\\n                    # 解析值（处理字符串和数字）\\n                    str_value = raw_value\\n                    if cell_type == \\"s\\":\\n                        try:\\n                            s_idx = int(raw_value)\\n                            str_value = shared_strings[s_idx] if s_idx < len(shared_strings) else raw_value\\n                        except:\\n                            str_value = raw_value\\n                    # 商品编码特殊处理（确保数字能被解析）\\n                    if col_name == \\"商品编码\\":\\n                        str_value = raw_value  # 直接保留原始数字字符串\\n\\n                    # 空字符串转None\\n                    row_data[col_name] = str_value if str_value.strip() != \\"\\" else None\\n                    found = True\\n                    break\\n\\n            # 未找到该列单元格，视为None\\n            if not found:\\n                row_data[col_name] = None\\n\\n        raw_data[row_num] = row_data\\n\\n    # 6. 过滤全为空的行\\n    filtered_data: Dict[str, Dict[str, Any]] = {}\\n    for row_num, row_data in raw_data.items():\\n        all_null = all(value is None for value in row_data.values())\\n        if not all_null:\\n            filtered_data[row_num] = row_data\\n\\n    # 7. 统计有效行数\\n    a_column_count = len(filtered_data)\\n\\n    return {\\n        \\"row_count\\": a_column_count,\\n        \\"data\\": filtered_data\\n    }\\n\\n\\nasync def main(args: Args) -> Output:\\n    params = args.params\\n    excel = params[\\"input\\"]\\n\\n    columns = [\\"促销简称\\", \\"促销内容详细描述\\", \\"促销主题\\", \\"促销方式\\", \\"促销类型\\", \\"商品限制\\", \\"优惠设置\\", \\"限制时段\\",\\n               \\"适用门店\\", \\"商品组\\", \\"商品编码\\"]\\n\\n    res = extract_excel_data(excel, columns)\\n\\n    # 构建输出对象\\n    ret: Output = {\\n        \\"key0\\": res[\\"row_count\\"],\\n        \\"data\\": res[\\"data\\"]\\n    }\\n    return ret","language":3,"settingOnError":{"processType":1,"timeoutMs":60000,"retryTimes":0}},"outputs":[{"type":"integer","name":"key0"},{"type":"object","name":"data","schema":[]}]}},{"id":"148929","type":"5","meta":{"position":{"x":818.45,"y":-13.700000000000017}},"data":{"nodeMeta":{"title":"代码_1","icon":"https://lf3-static.bytednsdoc.com/obj/eden-cn/dvsmryvd_avi_dvsm/ljhwZthlaukjlkulzlp/icon/icon-Code-v2.jpg","description":"编写代码，处理输入变量来生成返回值","mainColor":"#00B2B2","subTitle":"代码"},"inputs":{"inputParameters":[{"name":"data","input":{"type":"object","schema":[],"value":{"type":"ref","content":{"source":"block-output","blockID":"109520","name":"data"},"rawMeta":{"type":6}}}},{"name":"sandbox_id","input":{"type":"string","value":{"type":"ref","content":{"source":"block-output","blockID":"100001","name":"sandbox_id"},"rawMeta":{"type":1}}}},{"name":"url","input":{"type":"string","value":{"type":"ref","content":{"source":"block-output","blockID":"100001","name":"url"},"rawMeta":{"type":1}}}}],"code":"from urllib import request\\nfrom urllib import error\\nimport json\\nimport urllib.parse\\nimport time\\n\\n\\ndef SendTask(i, excel, sandbox_id, url):\\n    fill = excel[i]\\n\\n    user_prompt = f\\"**请严格操作，不可以更改顺序** \\\\n1. 下列数据是你需要一一对应填写到网页的，如果为None，你不需要填写```\\\\n[促销主题 : \\\\\\"{fill[\\"促销主题\\"]}\\\\\\",\\\\n促销简称 : \\\\\\n          \\\\\\"{fill[\\"促销简称\\"]}\\\\\\",\\\\n优惠设置 : \\\\\\"{fill[\\"优惠设置\\"]}\\\\\\",\\\\n限制时段 : \\\\\\"{fill[\\"限制时段\\"]}\\\\\\",\\\\n适用门店 : \\\\\\"{fill[\\"适用门店\\"]}\\\\\\",\\\\n促销详细内容 : \\\\\\"{fill[\\"促销内容详细描述\\"]}\\\\\\",\\\\\\n          \\\\n商品组 : \\\\\\"{fill[\\"商品组\\"]}\\\\\\",\\\\n商品编码 : \\\\\\"{fill[\\"商品编码\\"]}\\\\\\"\\\\n]  \\\\\\n          \\\\n2. 前往网页，点击最上面的“新增促销”。将刚刚输出的除了商品组和商品编码的内容按照先后顺序全选（Ctrl A）后填写替换。注意：如果刚刚输出的内容是None，不进行填写。\\\\\\n          \\\\n3. 检查：如果输出的商品编码是None，我们就不需要填写商品组了，你要做的就是不要执行步骤4以及步骤4后面的步骤了，你只需要将页面滚动到最下面，点击保存，随后结束整个流程；如果商品编码是 \\\\\\n          别的内容，你就不能执行商品编码是None的流程，你仍然需要按计划执行第四步以及后面的所有步骤。\\\\n4. 注意：不要填写商品组，不要填写商品组！ \\\\\\n          商品组的框是不能填写的！你的任务是：判断页面是否有和输出的商品组完全一致的的商品组；你需要做的是：持续向下滚动，找到或者找不到完全一致的商品组； \\\\\\n          什么是找不到：当你发现了右下角的保存按钮却没有发现对应的商品组时，说明没有对应商品组，这时你需要直接去执行第8步。\\\\\\n          5. 如果你发现了完全一致的商品组，首先，判断是否可以看到这个商品组的新建按钮。如果看不到，向下滚动一次。点击新建按钮 \\\\\\n          6. 其次，判断是否可以看到货类框下的BNC字样。如果看不到，向下滚动一次。\\\\\\n          7. 点击BNC右侧的商品编码框，填入商品编码\\\\n \\\\\\n          8. 将页面滚动到最下面，点击保存。注意，不要点x关闭。\\\\n\\"\\n\\n    try:\\n        # 发送 POST 请求\\n        # 要发送的数据，需要转换为字节流\\n        data = {\\n            \\"is_interrupt_feedback\\": False,\\n            \\"model_name\\": \\"doubao-1-5-thinking-vision-pro-250428\\",\\n            \\"sandbox_id\\": sandbox_id,\\n            \\"system_prompt\\": \\"\\",\\n            \\"task_mode\\": \\"operator\\",\\n            \\"thinking_type\\": \\"enabled\\",\\n            \\"user_prompt\\": user_prompt\\n        }\\n        # 处理数据：确保布尔值正确转换（urlencode会自动处理，但显式转换更稳妥）\\n        data_bytes = json.dumps(data).encode(\\"utf-8\\")\\n\\n        # 创建请求对象，显式指定请求头\\n        headers = {\\n            \\"Content-Type\\": \\"application/json; charset=utf-8\\",\\n            \\"User-Agent\\": \\"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Safari/537.36\\"\\n        }\\n        req = urllib.request.Request(url, data=data_bytes, method=\\"POST\\", headers=headers)\\n\\n        # 发送请求\\n        with urllib.request.urlopen(req) as response:\\n            content = response.read().decode(\\"utf-8\\")\\n\\n\\n    except urllib.error.HTTPError as e:\\n        print(f\\"HTTP 错误: {e.code}，响应内容: {e.read().decode(\\"utf-8\\")}\\")  # 打印服务器返回的错误详情\\n    except urllib.error.URLError as e:\\n        print(f\\"URL 错误: {e.reason}\\")\\n    except Exception as e:\\n        print(f\\"其他错误: {str(e)}\\")\\n\\n\\nasync def main(args: Args) -> Output:\\n    params = args.params\\n    excel = params[\\"data\\"]\\n    sandbox_id = params[\\"sandbox_id\\"]\\n    url = params[\\"url\\"]\\n    keys = list(excel.keys())\\n    for i in range(len(keys)):\\n        keys[i] = int(keys[i])\\n    keys.sort()\\n    for i in keys:\\n        SendTask(str(i), excel, sandbox_id, url)\\n\\n    # 构建输出对象\\n    ret: Output = {\\n        \\"key0\\": 0\\n    }\\n    return ret","language":3,"settingOnError":{"processType":1,"timeoutMs":60000,"retryTimes":0}},"outputs":[{"type":"integer","name":"key0"}]}}],"edges":[{"sourceNodeID":"100001","targetNodeID":"109520"},{"sourceNodeID":"148929","targetNodeID":"900001"},{"sourceNodeID":"109520","targetNodeID":"148929"}],"versions":{"loop":"v2"}}',
'[{"name":"input","type":"file","file_type":"excel","required":true},{"name":"sandbox_id","type":"string","required":true}]',
'[{"name":"output","type":"string"}]',1, 0, null, null, '1')
ON DUPLICATE KEY UPDATE
id = VALUES(id);